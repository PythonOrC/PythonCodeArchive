...
class Word(QMainWindow, Ui_word):
    ...
    def select_file(self):
        ...
        if file_path[0]:
            ...
            self.file_loc = self.file.split('/')[-1].split('.')[-2]
            self.get_words()
        else:
    ...
    def get_words(self):
        ...
        try:
            ...
            for word in words:
                ...
                meanings = meanings.replace('n.','').replace('adj.','').replace('vt.','').replace('adv.','').replace('vi.','').replace('v.','').replace(';','；').replace('，','；').split('；')
    ...
    def get_question(self):
        if len(self.words) == 0:
            ...
            with open(f'{self.file_loc} result.txt', 'a') as f:
                f.write('-------------------------------------\n\n')
                f.write('单词总数{}个， 回答正确{}个，正确率为{}%'.format(self.words_count, self.correct_count, int(self.correct_count/self.words_count*100)))
